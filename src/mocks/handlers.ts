...
export const handlers = [
  http.get('/api/accounts', () => {
    ...
    if (mode === 'accounts-error')
      return HttpResponse.json({ message: 'Accounts unavailable' }, { status: 500 });
  ...
  http.get('/api/accounts/:accountId/transactions', async ({ params }) => {
    ...
    const mode: string = getMockScenario();
    if ((mode === 'slow-account-a' || mode === 'out-of-order') && accountId === 'account-a')
      await delay(1500);
    if ((mode === 'slow-account-b' || mode === 'out-of-order') && accountId === 'account-b')
      await delay(100);
