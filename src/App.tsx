...
function App() {
  ...
  const loadAccounts = useCallback(async () => {
    ...
    try {
      const response = await checkedResponse('/api/accounts');
      const data = parseAccounts(await readJson(response));
      setAccounts({ status: 'success', data });
      setSelectedId((current) => current || data[0]?.id || '');
    } catch (error: unknown) {
      setAccounts({ status: 'error', error: toError(error) });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    void checkedResponse(`/api/accounts/${selectedId}/transactions`)
      ...
      .then((data) => {
        if (!controller.signal.aborted) setTransactions({ status: 'success', data });
      })
      .catch((error: unknown) => {
  ...
  return (
    <main>
      ...
      <h1>Account Switcher and Race Condition</h1>
      <section className="card stack">
        {accounts.status === 'loading' || accounts.status === 'idle' ? (
          <p role="status">Loading accounts…</p>
        ) : null}
        {accounts.status === 'error' ? (
          <div role="alert">
            <p className="error">Could not load accounts.</p>
            <button onClick={() => void loadAccounts()}>Retry accounts</button>
          </div>
        ) : null}
        ...
        {transactions.status === 'error' ? (
          <div role="alert">
            <p className="error">Could not load this account’s transactions.</p>
            <button onClick={() => setRequestVersion((value) => value + 1)}>
              Retry selected account
            </button>
          </div>
        ) : null}
